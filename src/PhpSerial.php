...
class PhpSerial
{
    ...
    public function __construct()
    {
        ...
        if (substr($sysName, 0, 5) === "Linux") {
            ...
            $this->_os = "windows";
            register_shutdown_function([$this, "deviceClose"]);
        } elseif (substr($sysName, 0, 6) === "CYGWIN") {
            $this->_os = "linux";
            register_shutdown_function([$this, "deviceClose"]);
        } else {
    ...
    public function deviceSet($device)
    {
        if ($this->_dState !== SERIAL_DEVICE_OPENED) {
            ...
            $t_os = $this->_os;
            trigger_error("Specified serial port is not valid $t_os, $device ", E_USER_WARNING);
     ...
     * @return bool
     */
    public function deviceOpen($mode = "r+b",$blockMode=0)
    {
        if ($this->_dState === SERIAL_DEVICE_OPENED) {
        ...
        echo "$this->_device" . " , $mode \r\n";
        //var_dump($this->_device);
        if ($this->_dHandle !== false) {
            stream_set_blocking($this->_dHandle, $blockMode);
    ...
    public function confParity($parity)
    {
        ...
        if ($this->_os === "linux") {
            ...
            echo "stty -F " . $this->_device . " " ;
            echo ", parity:";
            //var_dump( $args[$parity] );
        } elseif ($this->_os === "osx") {
            $ret = $this->_exec(
                "stty -f " . $this->_device . " " . $args[$parity],
                $out
    ...
    public function confCharacterLength($int)
    {
        ...
        if ($this->_os === "linux") {
            ...
            echo "stty -F " . $this->_device . " cs" . $int ;
    ...
    public function confStopBits($length)
    {
        ...
        if ($this->_os === "linux") {
            ...
            echo "stopbit: stty -F " . $this->_device . " " .
                (($length == 1) ? "-" : "") . "cstopb";
    ...
    public function confFlowControl($mode)
    {
        ...
        if ($this->_os === "linux") {
            ...
            echo "set flowControl: stty -F " . $this->_device . " " . $linuxModes[$mode];
    ...
    public function setSetserialFlag($param, $arg = "")
    {
        ...
        echo "set serial: setserial " . $this->_device . " " . $param . " " . $arg ;
