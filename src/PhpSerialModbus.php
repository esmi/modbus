...
//use Csgt\PhpSerial\PhpSerial;
use Esmi\modbus\PhpSerial;
...
class PhpSerialModbus
{
	...
	public function deviceInit($port='/dev/ttyUSB0', $baud=9600, $parity='none', $char=8, $sbits=1, $flow='none')
	{
		...
		//exec("mode COM3 BAUD=9600 PARITY=N data=8 stop=1 octs=off rts=on");
		echo $this->serial->_dState;
	...
	public function sendRawQuery ($string, $response = true) {
		$this->serial->sendMessage($string,0);
	...
	public function sendQuery ($slaveId, $functionCode, $registerAddress, $regCountOrData, $response = true)
	{
		...
		//if ($functionCode == 5) $regCountOrData = dechex($regCountOrData);
		$regCountOrData = dechex($regCountOrData);
		...
		$this->serial->sendMessage($queryString,0);
	...
	public function getResponse ($raw=false,$offsetl=0,$offsetr=0)
	{
		...
		if ($stringLength>=5)
		{
			...
			// $a = array_map("bin2hex",$responseData);
			// echo 'array_map("bin2hex",):';
			// var_dump($a);
			// return $a;
			// var_dump($responseData);
			$rts = array_map("bin2hex",$responseData);
			//var_dump($rts);
			return $rts;;
		} else
			{
			if ($this->debug) print "DEBUG [no response][$responseString]\n";
			return 0;
			}
		return 0;
	}
